...
export function initRandomPlaybackScreen(newElements) {
  ...
  elements.startButton.addEventListener("click", () => {
    ...
    elements.onStart(questionCountValue, categoryFilterValue);
  });
}
...
// 【2026-09-15追加、本人指示】通常ランダム再生の内部ルール。通常イントロ／アウトロと同じ「通常クイズ」として
// 出題数・カテゴリだけを選ぶ画面に揃えたため、ルール選択は無くした。タイムアタックのエンジンを流用している
// 都合上、内部的には normal（間違えた選択肢が消えて選び直せる。正解するまで／スキップ・答えを見るまで続く）で
// 固定する。normal を選ぶ理由：既存の自己ベスト（randomPlaybackBest.normal.*）とそのまま連続させるため。
// この値は互換のための内部値であり、結果画面・履歴・ランキングには「ルール」として表示・送信しない。
// タイムアタック／正解数バトル／ノーミスチャレンジは、タイムアタック側の出題タイプ「ランダム再生」で引き続き選べる。
export const RANDOM_PLAYBACK_RULE = TIME_ATTACK_RULE.NORMAL;

export function startRandomPlaybackRun(questionCountValue, categoryFilterValue) {
  const rule = RANDOM_PLAYBACK_RULE;
...
export function renderRandomPlaybackResult(questionCountValue, categoryFilterValue) {
  ...
  // 内部ルールは常に RANDOM_PLAYBACK_RULE（normal）。自己ベストのキーにだけ使い、表示・送信には使わない。
  const rule = RANDOM_PLAYBACK_RULE;
  ...
  // 【2026-09-15】通常ランダム再生にはルール選択が無いため、ルール欄は表示しない（内部値を見せない）。
  if (resultElements.ruleStat) resultElements.ruleStat.hidden = true;
  ...
  if (isNewRecord) {
    // ランキングへは通常イントロ／アウトロと同じく rule:null で送る（source:"normal" は呼び出し側 js/main.js で付与）。
  ...
  savePlayHistoryEntry({
    ...
    details: {
      // rule は内部値（normal固定）のため履歴には残さない（履歴詳細に「ルール」行を出さない）。
      totalElapsedMs: stats.totalElapsedMs,
      isNewRecord,
    },
  });
