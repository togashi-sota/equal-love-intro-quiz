// イントロ専用の苦手曲判定に使う、1問ごとの回答記録を集計するファイル。
//
...
//
// 【2026-08-30改訂・本人指示：苦手曲モードを5系統〈イントロ／アウトロ／シャッフル／リリック／
// 一瞬〉へ完全分離】このファイルは元々「通常イントロクイズ・イントロタイムアタック・
// 通常のランダム再生クイズ・ランダム再生タイムアタック」の4モード合算データだったが、
// 「シャッフル苦手曲を選んでも結局イントロ形式で出題される」「シャッフル単体の弱点が
// 分からない」という不具合につながっていたため、シャッフル分をjs/shuffleWeakSongStats.js
// （新設）へ完全に分離した。既存の合算データ（このファイルのlocalStorageキー）は
// 一切書き換えず、対象を「通常イントロクイズ・イントロタイムアタックの2モードのみ」へ
// 絞り込む形で引き続き使う（本人指示：「過去のデータから100%正確にモード別へ分離できない
// 場合は、推測で分離せず、今後のプレイからモード別記録を開始してよい」。既存の合算値には
// シャッフル分の記録も混ざっているが、削除・上書きはしていないため既存ユーザーの記録は
// 消えない。今後の記録だけが正確にイントロ専用になる）。
// 対象は次の2モードのみ：通常イントロクイズ・イントロタイムアタック。歌詞クイズ・
// アウトロ・シャッフル・一瞬チャレンジ・対戦モード・復習モード・苦手曲モード自身は対象外
// （呼び出し側でgameState.playMode・timeAttackScreen.jsのcurrentVariantを見て呼び分ける。
