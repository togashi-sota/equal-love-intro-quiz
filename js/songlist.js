...
// 【2026-09-15移設・本人指示：パーティー対戦の曲選択をオンライン対戦と同じ操作感へ】カテゴリバッジの定義
// （CATEGORY_PILL_INFO）と、シングルごとの区分け（resolveSongGroup／buildSongGroups）は、DOMに一切触れない
// js/songGrouping.js へ移した（normalizeForSearch を js/songSearch.js へ移したのと同じ理由：このファイルは
// モジュール読み込み時に DOM へ触れるため tests.html から import できない。共通部品 js/songGroupSelectList.js と
// そのテストが、DOM に依存しない置き場所を必要とした）。既存の呼び出し元（customQuizScreen.js・
// onlineBattleSongPicker.js 等）は引き続きこのファイルから import できるよう、ここで再export している（仕様・挙動は無変更）。
import { CATEGORY_PILL_INFO, buildSongGroups } from "./songGrouping.js";
export { CATEGORY_PILL_INFO, buildSongGroups };

// 【2026-09-15移設・本人指示：パーティー対戦の音声回答】曲名検索の正規化（normalizeForSearch）と
// 一致判定（songMatchesSearch）は、DOMに一切触れない純粋関数だけを集めたjs/songSearch.jsへ移した。
// 理由：このファイル（songlist.js）はモジュール読み込み時に#preview-audio等のDOM要素へ直接触れる
// ため、tests.htmlのようなDOMを持たない環境からimportできない。音声回答の曲名マッチング
// （js/songNameMatcher.js）はこの2関数を土台にしており、そちらをテストできるようにするには
// DOMに依存しない置き場所が必要だった。既存の呼び出し元（customQuizScreen.js・
// onlineBattleSongPicker.js・answerPoolBrowseUi.js・onlineLyricsQuizBattleScreen.js）は
// 引き続きこのファイルからimportできるよう、ここで再export している（仕様・挙動は無変更）。
import { normalizeForSearch, songMatchesSearch } from "./songSearch.js";
export { normalizeForSearch, songMatchesSearch };
