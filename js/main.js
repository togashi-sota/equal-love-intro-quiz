...
// ==========================================================================
// 【一時的な調査用コード】DEBUG_SAFE_AREA_INFO
// セーフエリアのCSS修正が実機でなぜ反映されないのかを確認するためだけの、
// 画面上部のデバッグ表示。歌詞本文・音源名・タイミングJSONなど著作権に関わる
// 情報は一切表示しない。原因特定後は、このブロックごと必ず削除すること。
// ==========================================================================
(async function DEBUG_SAFE_AREA_INFO() {
  const APP_BUILD_LABEL = "debug-v33";

  // env(safe-area-inset-top)の生の値を、ダミー要素のcomputedStyle経由で取得する
  ...
  const rawSafeAreaTop = getComputedStyle(probeElement).paddingTop;
  document.body.removeChild(probeElement);

  // Service Workerの登録状態（新しいバージョンが待機中のまま止まっていないか等）
  let serviceWorkerInfo = "Service Worker未対応";
  if ("serviceWorker" in navigator) {
    const registration = await navigator.serviceWorker.getRegistration();
    serviceWorkerInfo = registration
      ? `active=${!!registration.active} waiting=${!!registration.waiting} installing=${!!registration.installing} controller=${!!navigator.serviceWorker.controller}`
      : "登録なし";
  }

  const visualViewportInfo = window.visualViewport
    ? `${window.visualViewport.width}x${window.visualViewport.height} offsetTop=${window.visualViewport.offsetTop}`
    : "visualViewport未対応";

  const debugLines = [
    `build=${APP_BUILD_LABEL}`,
    `SW: ${serviceWorkerInfo}`,
    `body padding-top(計算値)=${getComputedStyle(document.body).paddingTop}`,
    `env(safe-area-inset-top)(生の値)=${rawSafeAreaTop}`,
  ...
  debugBarElement.style.cssText =
    "position:fixed; top:0; left:0; right:0; z-index:9999; background:#caff00; color:#000; " +
    "font-size:9px; line-height:1.35; padding:4px 6px; pointer-events:none; " +
    "white-space:pre-wrap; font-family:monospace; max-height:45vh; overflow:hidden;";
  debugBarElement.textContent = debugLines.join("\n");
